...
const i18n = {
  ...
  shareTitle:     ['શેર અને એક્સપોર્ટ કરો', 'Share and Export'],
  shareDesc:      ['ગણતરી પરિણામો કેવી રીતે શેર કરવા તેનું ઉદાહરણ', 'Example of how to share calculation results'],
  shareCardTitle: ['ઉદાહરણ આઉટપુટ', 'Example Output'],
  shareCardDesc:  ['એપ્લિકેશન દ્વારા જનરેટ થયેલ ઉદાહરણ', 'Example output generated by the application'],
  btnCopy:        ['કોપી કરો', 'Copy'],
  btnShare:       ['શેર કરો', 'Share'],
  btnPdf:         ['PDF એક્સપોર્ટ', 'Export PDF'],
  ancLabel:       ['ANC મુલાકાત સ્ક્રીન', 'ANC Visit Screen'],
  pncLabel:       ['PNC મુલાકાત સ્ક્રીન', 'PNC Visit Screen'],
...
function render() {
  ...
  setText('share-desc', 'shareDesc');
  setText('share-card-title', 'shareCardTitle');
  setText('share-card-desc', 'shareCardDesc');
  setHTML('btn-copy', ICONS.copy + ' ' + t('btnCopy'));
  setHTML('btn-share-el', ICONS.share + ' ' + t('btnShare'));
  setHTML('btn-pdf', ICONS.file + ' ' + t('btnPdf'));
  setText('anc-label', 'ancLabel');
  setText('pnc-label', 'pncLabel');
...
/* ── Copy to clipboard ── */
function copyText() {
  navigator.clipboard.writeText(EXAMPLE_TEXT).then(() => {
...
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('example-output').textContent = EXAMPLE_TEXT;
  render();
});
